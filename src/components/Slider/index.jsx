        ...
        <div className="slides">
            <div className="slide">
                {images.map((img, active) => {
                    return (
                        <img key={active} className={activeSlide === active ? "active" : ""} src={img.props.src}
                             alt=""/>
                    )
                })}
            </div>
            <div className="slides__buttons">
                <span className="slides__button-prev" onClick={() => prevSlide(activeSlide, images)}> &#8592;</span>
                <span className="slides__button-next" onClick={() => nextSlide(activeSlide, images)}>&#8594;</span>
            </div>
            <div className="slides__dots">
                {images.map((dot, index) => {
                    return (
                        <span onClick={() => setActiveSlide(index)} key={index}
                              className={activeSlide === index ? "active" : ""}></span>
                    )
                })}
            </div>
        </div>
